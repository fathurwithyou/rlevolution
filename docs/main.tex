...
\section{Metode}
Model penelitian terdiri atas empat komponen utama, yaitu populasi agen adaptif, lingkungan dua dimensi dengan makanan dan bahaya, mekanisme pembelajaran perilaku berbasis Q-learning, serta mekanisme evolusi berupa seleksi induk, crossover, dan mutasi. Implementasi dibuat modular melalui lapisan \texttt{domain/}, \texttt{rl/}, \texttt{environment/}, \texttt{evolution/}, \texttt{simulation/}, \texttt{visualization/}, dan \texttt{ui/}, sehingga simulasi, penyimpanan data, dan dashboard dapat dikembangkan secara terpisah. Hubungan antara arsitektur modul dan alur simulasi diringkas pada Gambar \ref{fig:system-flow}. Simulasi dijalankan sebagai eksperimen komputasional berulang sehingga perubahan fitness, survival, reproduksi, dan sifat populasi dapat diamati lintas generasi.
...
Lingkungan berbentuk bidang dua dimensi berukuran 100 x 72. Makanan disebar secara acak dan diganti setiap kali berhasil dikonsumsi, sedangkan zona bahaya berbentuk lingkaran dengan radius tertentu. Pada konfigurasi default dashboard, lingkungan berisi 85 makanan dengan radius dasar 2,4 dan 5 zona bahaya dengan radius 7,5. Agen kehilangan energi akibat metabolisme dan pergerakan dengan biaya dasar $0{,}32(1+0{,}38\|v\|s)/e$, memperoleh energi $28f$ ketika makan, dan kehilangan energi $18/e$ ketika berada di zona bahaya. Agen mati apabila energi habis atau umur melewati batas maksimum. Dengan demikian, setiap tindakan memiliki konsekuensi, misalnya bergerak dapat mendekatkan agen ke makanan atau pasangan, tetapi juga menghabiskan energi dan dapat membawa agen ke zona berbahaya.

State agen didiskretisasi menjadi 10 komponen, yaitu posisi $x$ dan $y$, arah dan jarak makanan terdekat, arah dan jarak bahaya terdekat, energi, dan umur. Representasi ini setara dengan \texttt{MultiDiscrete([5,5,3,3,5,3,3,5,5,5])} dengan ruang state maksimum 253.125 kombinasi. Aksi terdiri atas bergerak ke atas, bawah, kiri, kanan, diam, dan mencari pasangan reproduksi. Model Q-learning menggunakan aturan
\[
Q(s,a) \leftarrow Q(s,a) + \alpha \left(r + \gamma \max_{a'}Q(s',a') - Q(s,a)\right),
\]
dengan $\alpha=0{,}22$, $\gamma=0{,}92$, dan $\epsilon=0{,}18$ pada baseline. Pada konfigurasi default, eksplorasi meluruh mengikuti $\epsilon_t=\max(0{,}04,0{,}18\times0{,}985^{\mathrm{gen}})$. Reward positif diberikan untuk bertahan hidup ($+0{,}08$), memakan makanan ($+8{,}0$), dan reproduksi berhasil ($+18{,}0$), sementara reward negatif diberikan untuk bahaya ($-9{,}0$), kegagalan reproduksi, biaya pencarian pasangan, dan kematian ($-18{,}0$).
...
\]
dengan $R$ total reward, $A$ umur, $M$ jumlah makanan, $B$ jumlah reproduksi, $H$ paparan bahaya, dan $L=12$ jika agen masih hidup. Nilai akhir dibatasi dengan $F_{\mathrm{final}}=\max(0{,}1,F)$. Seleksi induk menggunakan fitness-proportionate selection dengan tekanan seleksi 1,35. Anak mewarisi rata-rata sifat dua induk, mengalami mutasi, dan menerima Q-table hasil crossover serta mutasi nilai Q.
...
\]
dengan $\mu$ laju mutasi dan $z$ salah satu trait biologis. Dua induk dipilih dengan replacement untuk setiap anak. Crossover Q-table dilakukan secara uniform, sedangkan mutasi nilai Q dilakukan per entri dengan peluang 0,12 dan noise $\mathcal{N}(0,0{,}08)$.

Dalam proses reproduksi, dua agen harus sama-sama hidup, memiliki energi minimal 58,0, berada dalam radius reproduksi 6,0, dan populasi masih kurang dari 90. Apabila syarat ini terpenuhi, peluang keberhasilan ditentukan oleh rata-rata sifat \textit{reproduction} kedua agen. Reproduksi mengurangi energi induk sebesar 20,0 dan energi pasangan sebesar 13,0, tetapi menambah individu baru dengan energi awal sekitar 42,16. Pada akhir generasi, populasi berikutnya dibentuk melalui seleksi induk berdasarkan fitness. Hal ini membedakan kelahiran langsung dalam episode dari regenerasi populasi antar generasi.
...
\centering
\textbf{(a) Alur simulasi}\\[3pt]
\begin{tikzpicture}[
  scale=0.82, transform shape,
  node distance=0.58cm,
  box/.style={rectangle,draw,rounded corners,align=center,minimum width=2.45cm,minimum height=0.46cm,font=\scriptsize},
  decision/.style={diamond,draw,aspect=2.15,align=center,inner sep=0.5pt,font=\scriptsize},
  arrow/.style={-{Latex[length=1.6mm]},thick}
]
\node[box] (init) {Populasi, makanan,\\bahaya};
\node[box,below=of init] (state) {Diskretisasi state};
\node[box,below=of state] (act) {Aksi $\epsilon$-greedy};
\node[box,below=of act] (env) {Reward dan energi};
\node[box,below=of env] (update) {Pembaruan Q-table};
\node[decision,below=of update,yshift=-0.03cm] (finish) {Episode\\selesai?};
\node[box,right=1.85cm of finish] (select) {Fitness dan\\seleksi};
...
\draw[arrow] (finish.west) -- ++(-0.7,0) |- node[left,pos=0.25]{Tidak} (state.west);
...
\begin{minipage}[t]{0.47\textwidth}
\centering
\textbf{(b) Arsitektur sistem}\\[3pt]
\vspace{0.32cm}
\begin{tikzpicture}[
  scale=0.82, transform shape,
  node distance=0.45cm,
  box/.style={rectangle,draw,rounded corners,align=center,minimum width=1.55cm,minimum height=0.42cm,font=\scriptsize},
  wide/.style={rectangle,draw,rounded corners,align=center,minimum width=4.45cm,minimum height=0.42cm,font=\scriptsize},
  arrow/.style={-{Latex[length=1.6mm]},thick}
]
\node[box,fill=gray!12] (config) at (0,0) {\texttt{config/}};
\node[box,fill=gray!12,right=1.0cm of config] (persist) {\texttt{persistence/}};
\node[box,fill=blue!10,below=0.6cm of config] (domain) {\texttt{domain/}};
\node[box,fill=blue!10,right=0.35cm of domain] (rl) {\texttt{rl/}};
\node[box,fill=blue!10,right=0.35cm of rl] (env) {\texttt{environment/}};
\node[box,fill=blue!10,right=0.35cm of env] (evol) {\texttt{evolution/}};
\node[wide,fill=green!10,below=0.65cm of rl,xshift=1.0cm] (engine) {\texttt{simulation/} -- EvolutionEngine};
\node[box,fill=orange!12,below=0.65cm of engine,xshift=-1.05cm] (viz) {\texttt{visualization/}};
\node[box,fill=orange!12,right=0.7cm of viz] (ui) {\texttt{ui/}};
\draw[arrow] (config) -- (domain);
\draw[arrow] (config) -- (rl);
\draw[arrow] (domain) -- (engine);
\draw[arrow] (rl) -- (engine);
\draw[arrow] (env) -- (engine);
\draw[arrow] (evol) -- (engine);
\draw[arrow] (engine) -- (persist);
\draw[arrow] (engine) -- (viz);
\draw[arrow] (viz) -- (ui);
\end{tikzpicture}
\end{minipage}
\caption{Arsitektur sistem dan alur simulasi adaptasi agen.}
\label{fig:system-flow}
\end{figure}

Mode dashboard menggunakan konfigurasi default 36 agen awal, 85 makanan, 5 bahaya, 24 generasi, dan 420 langkah per generasi, lalu menyimpan riwayat ke \texttt{data/experiment\_history.csv}. Laporan ini memakai hasil eksperimen yang sudah tersedia di \texttt{docs/experiments/rl\_evolution/}, yaitu 32 agen awal, 75 makanan, 12 generasi, 260 langkah per generasi, dan tiga seed (3211, 3212, 3213). Tujuh skenario diuji, yaitu baseline, mutasi rendah ($0{,}02$), mutasi tinggi ($0{,}30$), hazard rendah (2 zona), hazard tinggi (9 zona), tanpa eksplorasi ($\epsilon=0$), dan eksplorasi tinggi ($\epsilon=0{,}35$). Analisis dilakukan pada dua tingkat, yaitu agregasi lintas skenario dan dinamika temporal pada replikasi baseline.
